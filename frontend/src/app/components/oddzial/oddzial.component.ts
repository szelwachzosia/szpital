...
export class OddzialComponent implements OnInit {
  ...
  saveEdit() {
    ...
    this.http.put(`http://localhost:5001/api/oddzial/${this.selectedOddzial.oddzial_id}`, this.editForm)
      .subscribe({
  ...
  saveAdd() {
    this.http.post('http://localhost:5001/api/oddzial', this.addForm)
      .subscribe({
  ...
  confirmDelete(oddzial: any) {
    if (confirm(`Czy na pewno chcesz usunąć oddział: ${oddzial.nazwa}?`)) {
      this.http.delete(`http://localhost:5001/api/oddzial/${oddzial.oddzial_id}`)
        .subscribe({
